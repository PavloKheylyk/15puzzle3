...
ay = np.array(
    [
        [(ylist[0]), (ylist[0]), (ylist[0]), (ylist[0])],
        ...
        for y in range(4):
            if num[x, y] != 0 and result3 == False:
                pg.draw.rect(
                    screen,
                    (color[2]),
                    ((ax[x, y]), (ay[x, y]), size, size),
                    5,
                    10,
                )
                screen.blit(
                    my_font.render(str(num[x, y]), True, (color[2])),
                    ((ax[x, y] + 70), (ay[x, y] + 60)),
                )

            elif num[x, y] == 0:
                pg.draw.rect(
                    screen, (color[1]), ((ax[x, y]), (ay[x, y]), size, size), 5, 10
                )
                    ...
                    if event.type == pg.KEYDOWN:
                        ...
                        if event.key == QUIT or event.key == pg.K_ESCAPE:
                            ...
                            if result1 == True:
                                ...
                                screen.fill((color[0]))
                                screen.blit(
                                    my_font.render(("WIN!"), True, (color[2])),
                                    ((ax[x, y] + 70), (ay[x, y] + 60)),
                                )
